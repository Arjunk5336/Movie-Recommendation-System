...
def collaborative_recommendations(user_id, similarity, data, num_recommendations=5):
    ...
    recommended_movies = data[
        data['user_id'].isin(top_users) & ~data['title'].isin(user_movies)
    ]['title'].value_counts().head(num_recommendations)
...
def build_genre_matrix(movies):
    ...
    movie_genres = movie_genres.apply(pd.to_numeric, errors='coerce').fillna(0)
    return movie_genres
...
option = st.radio("Select Recommendation Type",
                  ("Collaborative", "Content-Based"))
data, movies = load_data()
movie_titles = sorted(movies["title"].unique())
movie_genres = build_genre_matrix(movies)  # Build once for efficiency

if option == "Collaborative":
    user_id = st.number_input("Enter User ID (1-943):",
                              min_value=1, max_value=943, step=1)
